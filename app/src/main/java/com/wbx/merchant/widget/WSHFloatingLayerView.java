...
public class WSHFloatingLayerView extends LinearLayout implements View.OnTouchListener {
    ...
    public WSHFloatingLayerView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        if(hasWindowFocus){
            ...
            /**
             * 每次滑动的距离是当前View宽度的5分之一。
             */
            move_height=floating_height/5;
        }
...
//                 */
                if(move_Y>down_Y&&down_move_Y>moveLength&&getCanAnimation()){
                    float[] values=new float[]{down_move_Y,getHeight()};
...
//        LogUtils.logd(getType()+"向下");
        onTopListener.isTop(false);
    }
...
//        LogUtils.logd(getType()+"向上");
        onTopListener.isTop(getType()==2);
    }
